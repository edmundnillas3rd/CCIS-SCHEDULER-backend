...
const creatorArchiveMeetings = async(req, res, next)=>{
    ...
    try{
        ...
        const [result] = await db.query(sql, [id]);
        if(!result.length){
           next(new ExpressError(404, "No Available Archive Meetings That You Created"))
        }else{
            let jsonObject = {}
            const key = "meetings"
            jsonObject[key] = []
        ...
        if(!result.length){
            ...
            if(!pass){
                ...
                if(result[0]?.isVerified === 0){
                    next(new ExpressError(400, "This Account Has Not Yet Been Veried, Please Check Your Email To Verify Your Account"))
                }else{
                    const token = jwt.sign({id: result[0]?.student_id}, process.env.ACCESS_TOKEN);
                    res.header(token)
                    res.status(200).json(
                        {success_message: "You successfully Login", student:{
                            id: result[0]?.student_id,
                            role: "student",
                            token: token
                        }})
...
const signup = async(req, res, next)=>{
    ...
    try{
        ...
        if(cjcemail[1] !== "g.cjc.edu.ph"){
            next(new ExpressError(400, "Please Use The Provided Email Of The School"))
        }else{
            let sql = `insert into students (
                ...
                const hashedPassword = await bcrypt.hash(password, 10);       
                 await db.query(sql, [id, fullname, course, year, email, username, hashedPassword])
                ...
                res.status(200).json({success_message: "Your account has been successfully created, Please Check Your Email To Verify Your Account"})
            }
    }catch(error){
       // console.log(error)
       if(error.errno === 1062 && error.sqlMessage == `Duplicate entry '${email}' for key 'students.students_email'`){
         next(new ExpressError(401, "Email already exist, provide new email"))    
       }
       if(error.errno === 1062 && error.sqlMessage == `Duplicate entry '${username}' for key 'students.students_username'`){
...
const verifySignup = async(req, res, next) => {
    try{
      ...
      if(!verifyJWT){
        ...
        await db.query(`set sql_safe_updates = 1`)
        res.status(200).json({success_message: "Your Account Has Been Verified, You Can Now Login Your Account"})
      }
    }catch(error){
        next(error)
    }
...
const createMeeting = async(req, res, next)=>{
    ...
    try{
        ...
        let sql = `insert into meetings (
         ...
         await db.query(sql, [meeting_id, code, title, description, date, day, start, end, venue_id, id]);  
        res.status(200).json({success_message: "You Succesfully Create A Meeting"})
    }catch(error){
        next(error)
    }
}
...
const deleteMeetingCreator = async(req, res)=>{
    try{
        ...
        res.status(200).json({success_message: "You Succesfully Delete The Meeting"})
    }catch(error){
        next(error)
    }
}
