...
const allStudent = async(req, res, next)=>{
    try{
       ...
       // console.log(result[0]?.student_image)
...
const login = async(req, res, next)=>{   
    try{
        ...
        if(!result.length){
            ...
            const token = jwt.sign({id: result[0]?.admin_id}, process.env.ACCESS_TOKEN);
            res.header(token)
            res.status(200).json(
                {success_message: "You successfully login", admin:{
...
const removeTeacher = async(req, res, next)=>{
   try{
    ...
    let sql = `delete from teachers where teacher_id = ?`;
     ...
     res.status(200).json({success_message: "Successfully Remove A Teacher"})
     }
   }catch(error){
    next(error)
   }
...
const removeStudent = async(req, res, next)=>{
    try{
        ...
        const [result] = await db.query(sql, id);
        if(result.affectedRows === 0){
            next(new ExpressError(404, "Please Remove Subadmin Privileges Of This Account"))
        }else{
...
const removeSubAdmins = async(req, res, next)=>{
    ...
    try{
        ...
        if(result.affectedRows === 0){
         ...
         res.status(200).json({success_message: "Successfully reomve A Subaddmin"})
         }
    }catch(error){
        next(error)
    }
